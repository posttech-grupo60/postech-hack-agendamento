...
import BuscarMedico from '@src/useCase/usuario/buscarMedico.usecase';
...
router.post('/buscar-medicos', async (req, res) => {
    try {
        const {especialidade, distancia, avaliacao, latLong} =  req.body;
        const usuarioRepository = new MongoDbUsuarioRepository();
        const buscarMedico = new BuscarMedico(usuarioRepository);
        const usuario = await buscarMedico.execute({especialidade, distancia, avaliacao, latLong});
        return res.status(200).json({data: usuario, status: 200});
    } catch (error: any) {
        console.log(error.message);
        return res.status(500).json({message: 'Erro! Contate a administração.', status: 500});
